...
function isAutoGeneratedLayoutDescription(description) {
  ...
  if (/Планировка с .+/i.test(text) && /высота потолков 3,1 м/i.test(text)) return true;
  if (/^(Однокомнатная|Двухкомнатная),/i.test(text) && /высота потолков 3,1 м/i.test(text)) return true;
  return false;
}
    ...
    sectors: targetProperty.sectors.map((sector) => ({
      ...
      flatVariants: (sector.flatVariants || []).map((variant) => {
        ...
        const layouts = getVariantLayouts(variant).map((layout) => {
          ...
          if (!savedDescription || isAutoGeneratedLayoutDescription(savedDescription)) return layout;
...
const JK2_LAYOUT_DESCRIPTION_LINES = [
  (feature, apt) => `Удобная планировка с ${feature}${apt ? `, ${apt} в продаже` : ''}. Высота потолков 3,1 м.`,
  (feature, apt) => `Продуманное пространство с ${feature}${apt ? `, ${apt} в продаже` : ''}. Потолки 3,1 м.`,
  (feature, apt) => `Комфортная планировка с ${feature}${apt ? `. ${apt.charAt(0).toUpperCase()}${apt.slice(1)}` : ''}.`,
  (feature, apt) => `${feature.charAt(0).toUpperCase()}${feature.slice(1)}, логичная зонировка${apt ? `, ${apt} в продаже` : ''}.`,
  (feature, apt) => `Светлая планировка с ${feature}${apt ? `, ${apt}` : ''}, высота потолков 3,1 м.`,
  (feature, apt) => `Функциональная планировка: ${feature}${apt ? `, ${apt} в продаже` : ''}.`,
  (feature, apt) => `Просторная планировка с ${feature}${apt ? `. ${apt.charAt(0).toUpperCase()}${apt.slice(1)}` : ''}, потолки 3,1 м.`,
  (feature, apt) => `Удачная планировка с ${feature}${apt ? `, ${apt}` : ''}. Высота потолков 3,1 м.`,
];
...
function buildJk2LayoutDescription(sectorTitle, flatType, layout, variant) {
  ...
  const hash = hashLayoutDescriptionKey(`${flatType}|${layout?.key}|${layoutLabel}|${areaLabel}|${apartmentsLabel}`);
  const feature = JK2_LAYOUT_DESCRIPTION_FEATURES[hash % JK2_LAYOUT_DESCRIPTION_FEATURES.length];
  const line1 = `${roomLabel}${areaLabel ? `, ${areaLabel}` : ''}.`;
  const line2 = JK2_LAYOUT_DESCRIPTION_LINES[hash % JK2_LAYOUT_DESCRIPTION_LINES.length](feature, apartmentsLabel);
